// official-geo-svg-traffic — replica of https://echarts.apache.org/examples/zh/editor.html?c=geo-svg-traffic
// title: GEO SVG Traffic / titleCN: 交通（SVG）
// An airport apron/taxiway plan (ksia-ext-plan-min.svg) registered as an SVG map
// (`registerMap(name, { svg })`), with three `lines` polylines drawn on `coordinateSystem: 'geo'` in the
// SVG's own coordinate space. `lineStyle.width: 0` makes the routes THEMSELVES invisible: the only visible
...
// The Kansai-style airport extension plan, vendored from the official asset tree
// (echarts-examples/public/data/asset/geo/ksia-ext-plan-min.svg). Parsed by zrender's parseSVG (ported into
// ZRenderKit) and wrapped as a geo coordinate system by GeoSVGResource. Read ONCE; a read failure degrades
// to an empty SVG (the pane renders blank rather than crashing).
private let ksiaExtPlanSVG: String = {
    let url = Upstream.repoRoot.appendingPathComponent("assets/geo/ksia-ext-plan-min.svg")
...
// The `effect.symbol`: a top-down car glyph, verbatim from the official example.
private let trafficCarSymbol = "path://M87.1667 3.8333L80.5.5h-60l-6.6667 3.3333L.5 70.5v130l10 10h80l10 -10v-130zM15.5 190.5l15 -20h40l15 20zm75 -65l-15 5v35l15 15zm-80 0l15 5v35l-15 15zm65 0l15 -5v-40l-15 20zm-50 0l-15 -5v-40l15 20zm 65,-55 -15,25 c -15,-5 -35,-5 -50,0 l -15,-25 c 25,-15 55,-15 80,0 z"

// Route 1 (red car, constantSpeed 100) — 38 points in the SVG's own coordinate space.
private let trafficRoute1Coords: [[Double]] = [
...
// Route 2 (blue car, constantSpeed 80) — 29 points.
private let trafficRoute2Coords: [[Double]] = [
...
// Route 3 (amber car, constantSpeed 60) — 23 points.
private let trafficRoute3Coords: [[Double]] = [
...
// series[0].data — one item per route, each overriding the series-level `effect` with its own colour,
// speed and delay.
private let trafficRoutes: [[String: Any]] = [
    [
        "effect": ["color": "#a10000", "constantSpeed": 100.0, "delay": 0.0] as [String: Any],
        "coords": trafficRoute1Coords
    ] as [String: Any],
    [
        "effect": ["color": "#00067d", "constantSpeed": 80.0, "delay": 0.0] as [String: Any],
        "coords": trafficRoute2Coords
    ] as [String: Any],
    [
        "effect": ["color": "#997405", "constantSpeed": 60.0, "delay": 0.0] as [String: Any],
        "coords": trafficRoute3Coords
    ] as [String: Any]
]

extension EChartsDemoRegistry {
    static let official_geo_svg_traffic = EChartsDemo(
        name: "official-geo-svg-traffic", category: "map",
        summary: "交通（SVG） — GEO SVG Traffic",
        width: 720, height: 460,
        nativeSupported: true,
        mapRegistrations: ["ksia-ext-plan": ["svg": ksiaExtPlanSVG] as [String: Any]],
        collection: .official,
        webOptionJS: #"""
option = {
  tooltip: {},
...
"""#,
        option: {
            ...
            // `echarts.registerMap('ksia-ext-plan', { svg: svg })` inside the $.get callback.
            ECharts.registerMap("ksia-ext-plan", ["svg": ksiaExtPlanSVG] as [String: Any])
            return [
                ...
                "series": [
                    [
                        ...
                        "polyline": true,
                        // width 0 — the routes are invisible; only the `effect` cars are drawn.
                        "lineStyle": [
                            "color": "#c46e54",
                            "width": 0.0
                        ] as [String: Any],
                        "effect": [
                            ...
                            "period": 8.0,
                            "color": "#a10000",
                            // upstream leaves `// constantSpeed: 80` commented out here — each data item
                            // supplies its own constantSpeed instead.
                            "trailLength": 0.0,
                            "symbolSize": [12.0, 30.0],
                            "symbol": trafficCarSymbol
                        ] as [String: Any],
                        "z": 100.0,
                        "data": trafficRoutes as [Any]
                    ] as [String: Any]
                ]
            ]
        }())
