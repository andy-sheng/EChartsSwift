// official-scatter-linear-regression — replica of
// https://echarts.apache.org/examples/zh/editor.html?c=scatter-linear-regression
// title: Linear Regression / titleCN: 线性回归（使用统计插件）
//
// 200 raw (x, y) samples drawn as a `scatter` series over `dataset[0]`, plus a `line` series fed by
// `dataset[1]`, whose `ecStat:regression` transform (the third-party echarts-stat plugin) least-squares-fits
// the points and returns them as the fitted line PLUS a third dimension carrying the formula text
// ("y = 1.7x + 3.01" for this data) on the LAST point only — printed at the line's end via
// `encode: { label: 2 }` + `labelLayout: { dx: -20 }`.
//
...
//     the whole page).
//   - Nothing else: the option is static (no timers, no re-setOption) and contains no closures — even
//     `symbolSize: 0.1` is a plain number — so nothing had to be omitted from the native option either.
//
...
// it only ever registers the two BUILT-INS, `filter` and `sort` (component/transform/transformInstall.swift).
// `ecStat:regression` belongs to echarts-stat — a separate JS plugin, outside the echarts source tree, so it
// has no Swift port and nobody registers that type. `applyDataTransform` therefore throws
// `Can not find transform on type "ecStat:regression".` while building `dataset[1]`, which kills the render
// before either series is laid out (not just the line). Exactly the shape of gap official-boxplot-light-velocity
// documents: the fix is a thin `ExternalDataTransform` (least-squares fit + the `y = <gradient>x + <intercept>`
// expression string written into dim 2 of the last point, per ecStat's `formulaOn: 'end'` default) plus one
...
// that is missing its regression line, which is the honest failure for a reference pane).
...
// The 200 (x, y) samples — upstream's `const data`, verbatim. Hoisted out of the option literal with an
// explicit type: a 200-element untyped nested literal is exactly what times out Swift's type-checker.
private let scatterLinearRegressionData: [[Double]] = [
    [0.067732, 3.176513],
    [0.42781, 3.816464],
    [0.995731, 4.550095],
    [0.738336, 4.256571],
...
extension EChartsDemoRegistry {
    static let official_scatter_linear_regression = EChartsDemo(
...
"""#,
        option: [
            "dataset": [
                // [0] the raw 200 (x, y) samples, consumed by the `scatter` series (dataset index 0 by default).
                ["source": scatterLinearRegressionData] as [String: Any],
                // [1] the ecStat regression fit. `config` is omitted upstream, so ecStat's defaults apply:
                // method 'linear', formulaOn 'end' (the formula string lands in dim 2 of the LAST point).
                [
                    "transform": [
                        "type": "ecStat:regression"
                    ] as [String: Any]
            ...
            "series": [
                ...
                [
                    ...
                    "type": "line",
                    "datasetIndex": 1.0,
                    // A number upstream (not a symbolSize closure) — the fitted line is drawn with its
