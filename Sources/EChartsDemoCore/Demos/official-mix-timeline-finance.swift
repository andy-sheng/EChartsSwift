// official-mix-timeline-finance — replica of https://echarts.apache.org/examples/zh/editor.html?c=mix-timeline-finance
// title: Finance Indices 2002 / titleCN: 2002全国宏观经济指标
// A timeline (2002…2011) over a `{ baseOption, options: [...] }` bundle: six province-keyed bar series
// (GDP / 金融 / 房地产 / 第一产业 / 第二产业 / 第三产业, the first three legend-deselected) plus a
// three-slice pie of the industry sums, re-fed per year by the timeline playhead.
//
// DEVIATIONS from the official source:
//   - timeline.autoPlay: the official option auto-advances every 1000ms. The gallery renders ONE static
//     frame, so BOTH panes pin the playhead to its initial index (2002) — the JS literal is verbatim and
//     `autoPlay` is switched off on the line after it (see the DEVIATION block at the bottom of webOptionJS).
...
//     pane would otherwise snapshot a half-grown bar animation. Turned off inside `baseOption` instead —
...
extension EChartsDemoRegistry {
    static let official_mix_timeline_finance = EChartsDemo(
        ...
        summary: "2002全国宏观经济指标 — Finance Indices 2002",
...
// reference pane on a single deterministic frame — the timeline's first index, unanimated — which is
// what the native pane renders. `option.animation = false` (injected by WebPage.swift) is ignored when
        ...
        option: [
            "baseOption": [
                ...
                "legend": [
                    "left": "right",
                    "data": ["第一产业", "第二产业", "第三产业", "GDP", "金融", "房地产"],
                    "selected": ["GDP": false, "金融": false, "房地产": false] as [String: Any]
                ] as [String: Any],
                "calculable": true,
                "grid": [
                    "top": 80.0,
                ...
                "xAxis": [
                    [
                        ...
                        "data": financeXAxisLabels,
                        "splitLine": ["show": false] as [String: Any]
                    ] as [String: Any]
                ],
                "yAxis": [
                    ["type": "value", "name": "GDP（亿元）"] as [String: Any]
                ],
                "series": [
                    ["name": "GDP", "type": "bar"] as [String: Any],
                    ["name": "金融", "type": "bar"] as [String: Any],
                    ["name": "房地产", "type": "bar"] as [String: Any],
                    ["name": "第一产业", "type": "bar"] as [String: Any],
                    ["name": "第二产业", "type": "bar"] as [String: Any],
                    ["name": "第三产业", "type": "bar"] as [String: Any],
                    [
                        "name": "GDP占比",
                        "type": "pie",
                        "center": ["75%", "35%"],
            ...
            "options": financeTimelineOptions
        ])
}

// The timeline's ten ticks. Two of them are objects in the official source (a bigger diamond symbol +
// their own tooltip); the 2005 tooltip is a string template and survives the port, the 2011 one is a
// closure and does not.
private let financeTimelineData: [Any] = [
    "2002-01-01",
    "2003-01-01",
    "2004-01-01",
    [
        "value": "2005-01-01",
        "tooltip": ["formatter": "{b} GDP达到一个高度"] as [String: Any],
        "symbol": "diamond",
        "symbolSize": 16.0
    ] as [String: Any],
    "2006-01-01",
...
// The 31 provinces, as the xAxis reads them: every other label is prefixed with a newline so the ticks
// stagger into two rows.
private let financeXAxisLabels: [String] = [
    "北京", "\n天津", "河北", "\n山西", "内蒙古", "\n辽宁", "吉林", "\n黑龙江", "上海", "\n江苏",
    "浙江", "\n安徽", "福建", "\n江西", "山东", "\n河南", "湖北", "\n湖南", "广东", "\n广西",
    "海南", "\n重庆", "四川", "\n贵州", "云南", "\n西藏", "陕西", "\n甘肃", "青海", "\n宁夏", "新疆"
]

// The `pList` of dataFormatter(), i.e. the same 31 provinces without the stagger newlines — every bar
// datum is named with one.
private let financeProvinces: [String] = [
    "北京", "天津", "河北", "山西", "内蒙古", "辽宁", "吉林", "黑龙江", "上海", "江苏",
    "浙江", "安徽", "福建", "江西", "山东", "河南", "湖北", "湖南", "广东", "广西",
    "海南", "重庆", "四川", "贵州", "云南", "西藏", "陕西", "甘肃", "青海", "宁夏", "新疆"
]

// dataFormatter(): `[v, ...]` -> `[{ name: pList[i], value: v }, ...]` (its `<year>max` output is unused
// by the option; its `<year>sum` is what the pie series eats — `financeSum` below).
private func financeItems(_ values: [Double]) -> [[String: Any]] {
    values.enumerated().map { ["name": financeProvinces[$0.offset], "value": $0.element] as [String: Any] }
}

private func financeSum(_ values: [Double]) -> Double {
    values.reduce(0, +)
}

// options[i] = the i-th timeline frame: title + the seven series' data for year 2002 + i.
private let financeTimelineOptions: [[String: Any]] = (0..<10).map { i -> [String: Any] in
    let year = 2002 + i
    return [
        "title": ["text": "\(year)全国宏观经济指标"] as [String: Any],
        "series": [
            ["data": financeItems(financeGDP[i])] as [String: Any],
            ["data": financeItems(financeFinancial[i])] as [String: Any],
            ["data": financeItems(financeEstate[i])] as [String: Any],
            ["data": financeItems(financePI[i])] as [String: Any],
            ["data": financeItems(financeSI[i])] as [String: Any],
            ["data": financeItems(financeTI[i])] as [String: Any],
            [
                "data": [
                    ["name": "第一产业", "value": financeSum(financePI[i])] as [String: Any],
                    ["name": "第二产业", "value": financeSum(financeSI[i])] as [String: Any],
                    ["name": "第三产业", "value": financeSum(financeTI[i])] as [String: Any]
                ]
            ] as [String: Any]
        ]
    ]
...
// The six indicator tables, one row per year, ascending 2002…2011 (the official source lists them
// descending), 31 provinces per row — in `financeProvinces` order.
private let financeGDP: [[Double]] = [
    /* 2002 */ [4315, 2150.76, 6018.28, 2324.8, 1940.94, 5458.22, 2348.54, 3637.2, 5741.03, 10606.85, 8003.67, 3519.72, 4467.55, 2450.48, 10275.5, 6035.48, 4212.82, 4151.54, 13502.42, 2523.73, 642.73, 2232.86, 4725.01, 1243.43, 2312.82, 162.04, 2253.39, 1232.03, 340.65, 377.16, 1612.6],
    /* 2003 */ [5007.21, 2578.03, 6921.29, 2855.23, 2388.38, 6002.54, 2662.08, 4057.4, 6694.23, 12442.87, 9705.02, 3923.11, 4983.67, 2807.41, 12078.15, 6867.7, 4757.45, 4659.99, 15844.64, 2821.11, 713.96, 2555.72, 5333.09, 1426.34, 2556.02, 185.09, 2587.72, 1399.83, 390.2, 445.36, 1886.35],
    /* 2004 */ [6033.21, 3110.97, 8477.63, 3571.37, 3041.07, 6672, 3122.01, 4750.6, 8072.83, 15003.6, 11648.7, 4759.3, 5763.35, 3456.7, 15021.84, 8553.79, 5633.24, 5641.94, 18864.62, 3433.5, 819.66, 3034.58, 6379.63, 1677.8, 3081.91, 220.34, 3175.58, 1688.49, 466.1, 537.11, 2209.09],
    /* 2005 */ [6969.52, 3905.64, 10012.11, 4230.53, 3905.03, 8047.26, 3620.27, 5513.7, 9247.66, 18598.69, 13417.68, 5350.17, 6554.69, 4056.76, 18366.87, 10587.42, 6590.19, 6596.1, 22557.37, 3984.1, 918.75, 3467.72, 7385.1, 2005.42, 3462.73, 248.8, 3933.72, 1933.98, 543.32, 612.61, 2604.19],
...
private let financeEstate: [[Double]] = [
    /* 2002 */ [298.02, 73.04, 140.89, 65.83, 51.48, 130.94, 76.11, 118.7, 384.86, 371.09, 360.63, 139.18, 188.09, 125.27, 371.13, 199.31, 145.17, 165.29, 808.16, 82.83, 21.45, 90.48, 210.82, 53.49, 95.68, 3.42, 77.68, 41.52, 9.74, 13.46, 43.04],
    /* 2003 */ [341.88, 92.31, 185.19, 78.73, 61.05, 188.49, 91.99, 127.2, 487.82, 447.47, 473.16, 162.63, 215.84, 138.02, 418.21, 217.58, 176.8, 186.49, 955.66, 100.93, 25.14, 113.69, 231.72, 59.86, 103.79, 4.35, 83.9, 48.09, 11.41, 16.85, 47.84],
    /* 2004 */ [436.11, 106.14, 231.08, 95.1, 73.81, 203.1, 97.93, 137.74, 666.3, 534.17, 587.83, 188.28, 248.44, 167.2, 473.27, 236.44, 204.8, 191.5, 1103.75, 122.52, 30.64, 129.12, 264.3, 68.3, 116.54, 5.8, 95.9, 56.84, 13, 20.78, 53.55],
    /* 2005 */ [493.73, 122.67, 330.87, 106, 98.75, 256.77, 112.29, 163.34, 715.97, 799.73, 688.86, 231.66, 331.8, 171.88, 664.9, 298.19, 217.17, 215.63, 1430.37, 165.05, 38.2, 143.88, 286.23, 76.38, 148.69, 10.02, 108.62, 63.78, 14.1, 22.97, 55.79],
...
private let financeFinancial: [[Double]] = [
    /* 2002 */ [561.91, 76.86, 179.6, 124.1, 48.39, 137.18, 75.45, 31.6, 485.25, 368.86, 347.53, 81.85, 138.28, 76.51, 310.07, 158.77, 96.95, 92.43, 454.65, 35.86, 10.08, 134.52, 183.13, 41.45, 102.39, 2.81, 67.3, 42.08, 16.75, 21.45, 52.18],
    /* 2003 */ [635.56, 112.79, 199.87, 118.48, 55.89, 145.38, 73.15, 32.2, 517.97, 392.11, 451.54, 87.45, 150.09, 64.31, 329.71, 165.11, 107.31, 99.35, 534.28, 61.59, 10.68, 147.04, 206.24, 48.01, 105.48, 4.74, 77.87, 42.31, 17.98, 24.8, 64.92],
    /* 2004 */ [713.79, 136.97, 209.1, 110.29, 55.89, 188.04, 77.17, 32.2, 612.45, 440.5, 523.49, 94.1, 171, 65.1, 343.37, 170.82, 118.85, 118.64, 602.68, 74, 11.56, 162.38, 236.5, 60.3, 118.4, 5.4, 90.1, 42.99, 19, 27.92, 70.3],
    /* 2005 */ [840.2, 147.4, 213.47, 135.07, 72.52, 232.85, 83.63, 35.03, 675.12, 492.4, 686.32, 127.05, 186.12, 69.55, 448.36, 181.74, 127.32, 162.37, 661.81, 91.93, 13.16, 185.18, 262.26, 73.67, 130.5, 7.57, 127.58, 44.73, 20.36, 32.25, 80.34],
