...
//   - NATIVE PANE OFF (nativeSupported: false). This example's CORE is the `renderItem` JS closure: the
//     calendar cells are EMPTY without it — every glyph on the canvas (the icons AND the day numbers) is
//     built there, from `api.coord` / `params.coordSys.cellWidth` / `echarts.number.linearMap` /
//     `echarts.format.formatTime` / `api.style` / `api.font`. A Swift `[String: Any]` option cannot carry
//     it, so only the echarts.js reference pane renders. (Nothing else in the option is exotic: the
//     calendar component and the custom series type are both ported.)
//
        ...
        option: [
            ...
            "series": [
                [
                    "type": "custom",
                    "coordinateSystem": "calendar",
                    // PORT-NOTE: series.renderItem omitted — the JS closure that IS this chart (hence
                    // nativeSupported: false). Per day cell it returns a `group` of:
                    ...
                    //     and positioned by `echarts.number.linearMap`-ing that event's `layouts` slot
                    ...
                    //   • one `text` element at the top of the cell: `echarts.format.formatTime('dd', …)`
                    //     — the day-of-month number — in `api.font({ fontSize: 14 })`, fill '#777'.
                    // Cells whose date falls outside `calendar.range` return early on the
