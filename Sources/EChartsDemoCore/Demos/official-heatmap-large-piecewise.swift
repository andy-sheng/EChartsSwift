// official-heatmap-large-piecewise — replica of https://echarts.apache.org/examples/zh/editor.html?c=heatmap-large-piecewise
// title: Heatmap - Discrete Mapping of Color / titleCN: 热力图 - 颜色的离散映射
// The same 201 x 101 cartesian heatmap (20,301 cells) as `heatmap-large`, but the visualMap is
// `type: 'piecewise'` — the 11-stop RdYlBu-ish ramp is QUANTISED into `splitNumber: 8` discrete
// buckets and rendered as a stacked legend of swatches (`left: 'right'`, `top: 'center'`), with
// `grid.right: 140` reserving the column it sits in. The cell values are a 2-D Perlin-noise field
...
//      seed is deterministic, so both panes here seed with the same constant (0.42 — the same one
//      `official-heatmap-large` pins, so the two demos also show the SAME field, only mapped
//      continuously vs. piecewise). That turns this demo from "not comparable by construction" into
//      a genuine pixel-for-pixel diff.
...
//      patterns; do the same for `heatmapPiecewiseData`; diff. (The two `perlin2`s agree because JS
//      `|` `<<` `>>` `&` are int32 ops — hence the Int32 coercion in `seed` — and `Math.floor`/`%`/
//      the float arithmetic map 1:1 onto `.rounded(.down)`/`%`/Double.)
...
//      Ditto `calculable: true` / `realtime: false`, which only matter while the visualMap's
//      selected pieces are being toggled by hand.
//
...
// `progressive: 1000` is honored on BOTH panes — natively it routes through
// HeatmapView.incrementalPrepareRender/incrementalRender (the cartesian branch is ported).
// No `drive`: the example assigns a static `option` and schedules nothing.
import Foundation

// MARK: - the upstream noisejs helper (https://github.com/josephg/noisejs), ported

private struct HeatmapPiecewiseGrad {
...
private let heatmapPiecewiseGrad3: [HeatmapPiecewiseGrad] = [
    HeatmapPiecewiseGrad(x: 1, y: 1, z: 0), HeatmapPiecewiseGrad(x: -1, y: 1, z: 0),
    HeatmapPiecewiseGrad(x: 1, y: -1, z: 0), HeatmapPiecewiseGrad(x: -1, y: -1, z: 0),
    HeatmapPiecewiseGrad(x: 1, y: 0, z: 1), HeatmapPiecewiseGrad(x: -1, y: 0, z: 1),
    HeatmapPiecewiseGrad(x: 1, y: 0, z: -1), HeatmapPiecewiseGrad(x: -1, y: 0, z: -1),
    HeatmapPiecewiseGrad(x: 0, y: 1, z: 1), HeatmapPiecewiseGrad(x: 0, y: -1, z: 1),
    HeatmapPiecewiseGrad(x: 0, y: 1, z: -1), HeatmapPiecewiseGrad(x: 0, y: -1, z: -1)
]

/// Ken Perlin's 256-entry permutation table, verbatim from the example's `p`.
private let heatmapPiecewisePermutation: [Int] = [
...
private struct HeatmapPiecewiseNoise {
    ...
    /// (Safe here: the only caller passes the literal `heatmapPiecewiseSeed`.)
    mutating func seed(_ s: Double) {
        ...
        for i in 0..<256 {
            let v = (i & 1) != 0
                ? heatmapPiecewisePermutation[i] ^ (Int(value) & 255)
                : heatmapPiecewisePermutation[i] ^ ((Int(value) >> 8) & 255)
            perm[i] = v
            perm[i + 256] = v
            gradP[i] = heatmapPiecewiseGrad3[v % 12]
            gradP[i + 256] = heatmapPiecewiseGrad3[v % 12]
        }
    }
...
private let heatmapPiecewiseSeed = 0.42
...
private let heatmapPiecewiseGenerated: (data: [[Double]], xData: [Double], yData: [Double]) = {
    let noise = HeatmapPiecewiseNoise(seed: heatmapPiecewiseSeed)
    var data: [[Double]] = []
    var xData: [Double] = []
    var yData: [Double] = []
...
private let heatmapPiecewiseData: [[Double]] = heatmapPiecewiseGenerated.data
private let heatmapPiecewiseXData: [Double] = heatmapPiecewiseGenerated.xData
private let heatmapPiecewiseYData: [Double] = heatmapPiecewiseGenerated.yData

/// The visualMap's 11-stop colour ramp — quantised into `splitNumber: 8` pieces by the piecewise
/// visualMap (it samples the ramp at each bucket's midpoint rather than showing all 11 stops).
private let heatmapPiecewiseColors: [String] = [
...
extension EChartsDemoRegistry {
    static let official_heatmap_large_piecewise = EChartsDemo(
        name: "official-heatmap-large-piecewise", category: "heatmap",
        summary: "热力图 - 颜色的离散映射 — Heatmap - Discrete Mapping of Color",
...
"""#,
        option: [
            ...
            "xAxis": [
                "type": "category",
                "data": heatmapPiecewiseXData
            ] as [String: Any],
            "yAxis": [
                "type": "category",
                "data": heatmapPiecewiseYData
            ] as [String: Any],
            "visualMap": [
                "type": "piecewise",
                "min": 0.0,
