// official-pictorialBar-spirit — replica of https://echarts.apache.org/examples/zh/editor.html?c=pictorialBar-spirit
// title: Spirits / titleCN: 精灵
// Two pictorialBar series on an inverse category y-axis (2013…2016), both repeating a base64 sprite
// (`symbolRepeat: 'fixed'`, `symbolBoundingData: 2000`): a CLIPPED "current" series (`symbolClip`, z:10)
// carrying a `max` markLine, drawn over a 20%-opacity "full" series (z:5) that labels each bar. Upstream
// re-randomises BOTH series' data every 3s.
//
// THE 3s RANDOMISATION IS PORTED. The web pane runs the example's own
// `setInterval(function () { myChart.setOption({ series: [{ data }, { data }] }); }, 3000)` verbatim (a
// MERGING setOption — it replaces only the two series' data); the `drive` closure below replays the
// SAME timeline on the native pane (see EChartsDemoChart). `drive` is written and kept, but dormant
// while `nativeSupported` is false — the gallery only calls it when it builds a native pane (see the
// NATIVE PANE note below), so it lights up together with the native pane and nothing has to be
// reconstructed then. The still-frame PNG paths capture the first frame (the [891, 1220, 660, 1670]
...
// `var maxData = 2000;`
private let pictorialBarSpiritMaxData: Double = 2000
...
"""#,
        // The native half of the same timeline: every 3s, feed BOTH series a fresh set of four random
        // values — upstream's `setInterval(..., 3000)`. A MERGING setOption (notMerge: false), exactly as
        // upstream: only `series[i].data` is replaced, the rest of the option stands.
        drive: { chart in
            // `function random() { return +(Math.random() * (maxData - 10)).toFixed(1); }`
            func random() -> Double {
                ((Double.random(in: 0 ..< (pictorialBarSpiritMaxData - 10))) * 10).rounded() / 10
            }
            chart.every(3) {
                let dynamicData = [random(), random(), random(), random()]
                chart.setOption([
                    "series": [
                        ["data": dynamicData] as [String: Any],
                        ["data": dynamicData] as [String: Any]
                    ]
                ], notMerge: false)
            }
        },
        option: pictorialBarSpiritOption)
}

private let pictorialBarSpiritOption: [String: Any] = [
    "tooltip": [:] as [String: Any],
    "xAxis": [
        "max": pictorialBarSpiritMaxData,
        "splitLine": ["show": false] as [String: Any],
        "offset": 10.0,
        "axisLine": [
            "lineStyle": ["color": "#999"] as [String: Any]
        ] as [String: Any],
        "axisLabel": ["margin": 10.0] as [String: Any]
    ] as [String: Any],
    "yAxis": [
        "data": ["2013", "2014", "2015", "2016"],
        "inverse": true,
    ...
    "series": [
        ...
        [
            "type": "pictorialBar",
            "symbol": pictorialBarSpiritSymbol,
            "symbolRepeat": "fixed",
            "symbolMargin": "5%",
            "symbolClip": true,
            "symbolSize": 30.0,
            "symbolBoundingData": pictorialBarSpiritMaxData,
            "data": pictorialBarSpiritData,
            "markLine": [
                "symbol": "none",
                "label": [
                    "formatter": "max: {c}",
