...
// PM2.5 of 190 Chinese cities on a `geo` map of china: a `scatter` series (all cities) + an
// `effectScatter` series (top 6 by value) both on coordinateSystem 'geo', a `brush` component wired to
// geoIndex 0, and an empty right-hand `bar`/category-yAxis pair that the brush handler fills in.
//
// DEVIATIONS from the official source:
//   - INTERACTION DROPPED. The example's payload lives in three statements after the option:
//     `myChart.on('brushselected', renderBrushed)`, a `setTimeout(… myChart.dispatchAction({type:'brush',
//     areas:[{ geoIndex: 0, brushType: 'polygon', coordRange: […190 points…] }]}) , 0)` that programmatically
//     brushes a Jiangsu/Zhejiang polygon, and `renderBrushed`, which re-setOptions the yAxis categories, the
//     'bar' series data and the 'statistic' title ('平均: …') from the selection. All three are gone from BOTH
//     panes: the gallery renders one static frame with no pointer input, `myChart` does not exist while the
//     option script runs, and — since the native pane can carry only an `option` — keeping the brush in the web
//     pane alone would make the two panes diverge and destroy the diff. Both panes therefore show the INITIAL
//     state: the geo + both scatter series, an empty bar series, an empty category yAxis and a blank
//     'statistic' title — i.e. the example before you drag (or it dispatches) a selection.
//   - MAP ASSET. The example assumes a 'china' map is already registered (the official editor injects it);
//     it is NOT in the echarts-examples asset mirror. assets/geo/china.json is the plain GeoJSON extracted
//     from the vendored upstream/echarts/test/data/map/js/china.js (42 province-level features, Apache-2.0,
//     the exact data `echarts.registerMap('china', …)` gets in upstream's own tests). It is declared in
//     `mapRegistrations` so BOTH panes register the same map (WebPage.swift injects registerMap into the page).
//   - NATIVE PANE: the two `symbolSize: function (val) { return Math.max(val[2] / 10, 8); }` closures cannot
//     cross into a Swift option — both scatter series fall back to the default symbol size (see PORT-NOTEs),
//     so the native bubbles are uniform where the web ones scale with PM2.5. Everything else is identical.
//   - TS types (`interface DataItem`, `Record<string, number[]>`) and the trailing `export {};` dropped.
import Foundation
import EChartsKit
        ...
        option: {
            ...
            return [
                ...
                "title": [
                    ...
                    [
                        // The brush handler would write '平均: …' here; static frame → no text.
                        "id": "statistic",
                        "right": 120.0,
                        "top": 40.0,
                        "width": 100.0,
                ...
                "yAxis": [
                    ...
                    // Filled by the dropped brush handler; empty in the initial state.
                    "data": [] as [Any]
                ] as [String: Any],
                "series": [
                    [
                        ...
                        // PORT-NOTE: symbolSize omitted — JS closure `function (val) { return Math.max(val[2] / 10, 8); }`
                        // sized each bubble by its PM2.5 (value[2]/10, floor 8px).
                        "label": [
                            "formatter": "{b}",
                            "position": "right",
                            "show": false
                    ...
                    [
                        ...
                        // PORT-NOTE: symbolSize omitted — same `Math.max(val[2] / 10, 8)` closure as above.
                        "showEffectOn": "emphasis",
