...
// A 10x10 category/category grid of scatter points (symbolSize 50, symbolKeepAspect,
// universalTransition) whose SYMBOL is swapped every 700ms by a setInterval that re-setOptions a
// new `symbol` (roundRect → circle → heart → happy → evil → hipster → shocked → pie → users → mug →
// plane), so the marks morph from one SVG path into the next.
//
// DEVIATIONS from the official source:
//   - ANIMATION REDUCED TO ITS FIRST FRAME. The gallery renders one static snapshot, so the
//     `setInterval(... myChart.setOption(options[i]) ..., 700)` driver is dropped from both panes;
//     both render `options[0]` only (the roundRect grid). The web pane still carries the full
//     `options` array verbatim so the morph targets stay visible in the reference source; the Swift
//     `option` carries only frame 0, since the later entries are re-setOption merge payloads that a
//     single static option cannot express.
//   - TypeScript-only `const options: echarts.EChartsOption[]` annotation stripped (the reference
//     page is a classic script), and the trailing `export {};` dropped.
//   - The 100 data points / the two axis category arrays are built by `for` loops in the official
//     source; the web pane keeps those loops verbatim, the Swift side computes the same arrays in
//     file-scope `private let`s.
//   - Canvas bumped to 560x560 (from the tab default 640x420): the grid has zero padding on all four
//     sides, so 10 rows of 50px symbols need ~500px of height or the marks overlap/clip.
...
// Static frame: the official source starts at options[0] and cycles through the rest on a
// setInterval (dropped — the gallery snapshots one frame).
option = options[0];
"""#,
        option: [
            "grid": [
...
// 10x10 grid of [x, y, value] triples — the official source builds these with nested `for` loops.
...
// Both category axes are 0...9 (hidden; they only lay out the grid).
