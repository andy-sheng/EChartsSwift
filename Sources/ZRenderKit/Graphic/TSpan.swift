...
//   → PORT-TODO: graphic/helper/parseText.ts is not ported yet. `tSpanHasStroke` is inlined
//     (trivial, same predicate as Path.hasStroke). `tSpanCreateBoundingRect` (text measurement) is
//     stubbed — see getBoundingRect.
...
// PORT-TODO: interface TSpanProps extends DisplayableProps { style?: TSpanStyleProps }. The
...
// PORT-TODO: TSpanState = Pick<TSpanProps, DisplayableStatePropNames>. States machinery is Phase 2;
//   collapsed onto Displayable's stub.
...
public final class TSpan: Displayable {
    ...
    internal override func _init(_ props: ElementProps? = nil) {  // upstream: protected
        // PORT-TODO: TSpan has no own `_init` upstream; it inherits Displayable._init, which routes
    ...
    public func hasStroke() -> Bool {
        ...
        // PORT-TODO: parseText.ts not ported; `tSpanHasStroke` inlined (stroke != null &&
    ...
    // blend. PORT-TODO: a Swift-only bridge — upstream has a single `this.style` object.
