...
public final class CompoundPath: Path {
    ...
    // upstream `CompoundPath` declares `shape: CompoundPathShape` but no `getDefaultShape`,
    ...
    // upstream signature `beforeBrush()` takes no args; the Swift base
    ...
    // upstream signature `buildPath(ctx, shape)` omits the `inBatch` arg; the Swift base
