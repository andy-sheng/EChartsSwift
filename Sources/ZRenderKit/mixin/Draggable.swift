...
// (Handler strongly owns its `_draggingMgr`, and Handler always outlives it). // note: verify capture
...
public protocol DraggableHandler: AnyObject {
    ...
    // `event` is the ZRRawEvent (browser DOM event), Any? at the native event seam.
...
public final class Draggable {
    ...
    // upstream listeners take `(e: ElementEvent)` directly; here they are bound through
