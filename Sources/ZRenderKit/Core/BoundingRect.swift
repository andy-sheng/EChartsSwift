...
// PORT-TODO: JS indexes Point/RectLike by these string keys (e.g. `_minTv[updateDim]`,
...
public final class BoundingRect: RectLike {
    ...
    public init(_ x: Double, _ y: Double, _ width: Double, _ height: Double) {
        // PORT-TODO: stored props must be initialized before passing `self` to
    ...
    public func plain() -> RectLike {
        // PORT-TODO: upstream returns an object literal { x, y, width, height } typed RectLike.
    ...
    public static func calculateTransform(_ out: MatrixArray?, _ a: RectLike, _ b: RectLike) -> MatrixArray {
        ...
        // PORT-TODO: upstream reuses `out || []` as scratch (`out = matrix.identity(out || [])`).
...
// PORT-TODO: dropped — `vector.set` is value-returning (CONVENTIONS §3), no scratch buffer needed.
...
// PORT-TODO: upstream RectLike is a structural type that plain objects also satisfy. We
 ...
 */
// PORT-TODO: upstream returns a closure-based object literal `_ctx` capturing `_direction`,
