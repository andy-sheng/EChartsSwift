...
//   (GradientObject / GradientColorStop). See the `extend({}, color)` note on `liftColor`.
...
/// `util.isFunction` note recommends. Literal conformances keep call sites close to
...
public enum color {
    ...
    // the original's error branches (`setRgba(rgbaArr,0,0,0,1); return;`) mutate
    ...
    static func stringOf(_ p: CssParam) -> String {
        ...
        // full JS Number→String fidelity (exponential form, etc.) not replicated.
    ...
    /// not a complete ECMAScript ToNumber (no 0x.. / Infinity literals, etc.).
    ...
    /// not full ECMAScript Number::toString (exponential notation, etc.).
    ...
    /// narrow subset (no exponent edge cases beyond the common forms).
