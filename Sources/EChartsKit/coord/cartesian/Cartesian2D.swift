...
// PORT-TODO (CONVENTIONS §2): module-level exported const kept as a top-level `public let`, matching the
...
open class Cartesian2D: Cartesian<Axis2D>, CoordinateSystem {
    ...
    // PORT-TODO: `CoordinateSystem.dimensions` is `{ get set }`, so this is a `var` (not the upstream
    ...
    // PORT-TODO: concrete upstream type is `Grid`; stored as the protocol type `CoordinateSystemMaster?`
    ...
    // PORT-TODO: concrete upstream type is `GridModel`; stored as the protocol type `ComponentModel?`
    ...
    // PORT-TODO: this is the `CoordinateSystem.dataToPoint` witness — its signature follows the ported
    ...
    // PORT-TODO: `CoordinateSystem.pointToData` witness — `opt: Any?` = upstream `clamp`, `out` dropped;
    ...
    // PORT-TODO: upstream param/return are `Axis2D`; kept faithful. Because the param is more specific
