...
//       PORT-TODO: `Parallel` (the 5th coordinate-system master) is a FORTHCOMING sibling — not ported in
//       this phase. `coordinateSystem` is typed via `CoordinateSystemMaster?` below (mirroring PolarModel);
//       narrow via `as? Parallel` once it lands.
//   import {
...
//   PORT-TODO: `ParallelCoordinateSystemOption` describes the dynamic option shape (mainType/layout/
...
public final class ParallelModel: ComponentModel, CoordinateSystemHostModel {
    ...
    //   PORT-TODO: upstream types this the concrete `Parallel` (a `CoordinateSystemMaster`), injected by
    //   parallelCreator once the coordinate system is built. `Parallel` (coord/parallel/Parallel.swift) is a
    //   forthcoming sibling; typed here as the `CoordinateSystemMaster?` required by
    ...
    public override func mergeOption(_ newOption: ModelOption?, _ ecModel: GlobalModel?) {
        ...
        //   PORT-TODO: upstream mutates `this.option` in place; Swift option bags are value types, so
    ...
    //   PORT-TODO: TS object-param modeled as the dynamic `[String: Any]` bag.
...
private func jsNumStr(_ v: Any?) -> String {
    ...
    // PORT-TODO: JS would stringify other values (e.g. `undefined` -> "undefined"); `dim` is always a
