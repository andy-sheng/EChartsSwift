...
//       PORT-TODO: the inherited `Axis.model: AxisBaseModel!` already provides the model slot — no
...
//       coordinate-system master). PORT-TODO: sibling landing with the parallel coord-system port; the
...
public final class ParallelAxis: Axis {
    ...
    //   coordinate-system port; typed here to that master so `getModel()` resolves. Narrow the inherited
    //   `model` via `as? ParallelAxisModel` at use.
