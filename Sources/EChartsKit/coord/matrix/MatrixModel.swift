...
// PORT-TODO: `Matrix` / `MatrixDim` / `MatrixBodyCorner` (coord/matrix/{Matrix,MatrixDim,MatrixBodyCorner}.swift)
//   are NOT ported yet — they are siblings in a later phase. This file references those forward types
//   (like CalendarModel references the forward `Calendar`), so it does not compile standalone (staged).
...
//   PORT-TODO: all of the above are TypeScript option/param interfaces describing the dynamic option
...
public final class MatrixModel: ComponentModel, CoordinateSystemHostModel {
    ...
    //   PORT-TODO: upstream types this the concrete `Matrix` (a `CoordinateSystemMaster`), injected once
    ...
    //   PORT-TODO: upstream parameterizes `MatrixBodyCorner` by the string-literal kind ('body' / 'corner').
    ...
    // optionUpdated(): void { ... }
    //   PORT-TODO: upstream overrides with a param-less `optionUpdated(): void`, narrowing the base
