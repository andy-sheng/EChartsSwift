...
// `MapOrGeoModel` is the union `GeoModel | MapSeries` (both are ComponentModel). Modeled as
...
func resizeGeo(_ geo: Geo, _ geoModel: MapOrGeoModel, _ api: ExtensionAPI) {
    ...
    if useCenterAndSize {
        ...
        // `aspect` is not a field of `BoxLayoutOptionMixin`; upstream augments the cast bag.
        ...
        var viewRectVar = layout.getLayoutRect(boxLayoutOption, refContainer)
...
public final class GeoCreator: CoordinateSystemCreator {
    ...
    // `Geo` now conforms to `CoordinateSystemMaster` (coord/geo/Geo.swift), so the `[Geo]` list
    ...
    public func getFilledRegions(
        ...
        // upstream `dataNameMap` stores the RegionOption OBJECT (JS reference) so the later
