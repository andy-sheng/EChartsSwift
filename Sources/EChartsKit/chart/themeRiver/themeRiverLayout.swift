...
//   PORT-TODO: modeled as a `[String: Any]` layout bag ("rect": BoundingRect, "boundaryGap": [Double])
...
public func themeRiverLayout(
    ...
    ecModel.eachSeriesByType(SERIES_TYPE_THEME_RIVER) { seriesModelBase, _ in
        ...
        //   PORT-TODO: coord/single/Single.swift lands with the Single coord-sys phase; `coordinateSystem`
        //   is the inherited `Any?` slot. `Single` exposes getRect() -> BoundingRect, getAxis() -> SingleAxis
        //   (with `.orient`), dataToPoint(_ value) -> [Double].
        ...
        //   PORT-TODO: `axis.orient` is a SingleAxis field (LayoutOrient), landing with the Single phase;
        //   `single.getAxis()` returns that SingleAxis, so `.orient` resolves once the sibling is present.
        if axis.orient == .horizontal {
