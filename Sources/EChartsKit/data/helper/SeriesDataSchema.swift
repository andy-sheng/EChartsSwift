...
// `inner` is a per-`Source` property store. Upstream:
...
public final class SeriesDataSchema {
     ...
     */
    // upstream `readonly` (binding fixed, but `appendCalculationDimension` pushes
    ...
    public func getSourceDimension(_ dimIndex: DimensionIndex) -> DimensionDefinition? {
        ...
        if let dimensionsDefine = dimensionsDefine {
            // JS returns `undefined` on out-of-range index; guard to avoid a trap.
            let i = Int(dimIndex)
    ...
    public func makeStoreSchema() -> (
        ...
        while Double(fullDimIdx) < dimCount {
            ...
            // JS `this.dimensions[seriesDimIdx]` yields `undefined` past the end;
            ...
            // relies on the sibling data/DataStore port exposing
    ...
    public func makeOutputDimensionNames() -> [DimensionName?] {
        // upstream return type is `DimensionName[]`, but `name` is left `undefined`
