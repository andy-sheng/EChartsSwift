...
public class BreakScaleMapper: ScaleMapper {
    ...
    public var breaks: ParsedAxisBreakList { [] }   // overridden by BreakScaleMapperImpl in scale/breakImpl.swift

    public func hasBreaks() -> Bool {
        return false                                // overridden by BreakScaleMapperImpl in scale/breakImpl.swift
    }
    ...
    public func calcNiceTickMultiple(
        ...
        return 0                                    // overridden by BreakScaleMapperImpl in scale/breakImpl.swift
    }
...
// the TS string-literal union (incl. its `NullUndefined` arm) is modeled as `String`
...
public protocol BreakScaleHelper {
    ...
    // the `TItem extends ScaleTick | number` constraint is a TS union and cannot be
    ...
    func parseAxisBreakOption(
        ...
        // upstream narrows to the structural `{parse}`; Swift's IUO `Scale.parse` stored
        ...
        // upstream's inline opt requires `noNegative`; reuse `ParseBreakOptionOpt` (optional field).
    ...
    //   ? TItem[][] : number[][]
    // the conditional return type (`TItem[][] | number[][]` keyed on `returnIdx`) cannot be
...
public func simplyParseBreakOption(
    // upstream: scale: {parse: Scale['parse']} (a structural subset of `Scale`).
    // upstream narrows to `{parse}`; we accept the concrete `Scale` (see
...
// ============================================================================
// Forward-declaration placeholder (note).
//
...
// forward-declared placeholder; replace with the real type in coord/axisCommonTypes.swift.
