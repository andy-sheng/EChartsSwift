...
//
// note (cross-sibling dependency): this file relies on the established mapper design in
...
public final class LogScale: Scale, ClassManageable {
    ...
    // upstream `readonly`; constructed after `super.init()` (it needs `self` and the parsed
    ...
    public init(_ setting: LogScaleSetting) {
        ...
        if let scaleBreakHelper = scaleBreakHelper {
            ...
            // upstream shares the `lookup` arrays by reference between `this._lookup` and
        ...
        return util.map(intervalStub.getTicks(opt ?? ScaleGetTicksOpt())) { [unowned self] tick, _ in
            ...
            if let scaleBreakHelper = scaleBreakHelper {
                ...
                if let brkPowResult = brkPowResult {
                    ...
                    // upstream `powVal = brkPowResult.tickVal` may assign `number|undefined`
    ...
    // the abstract base declares `getLabel(_ tick: ScaleTick) -> String`. Upstream satisfies
    ...
    // upstream is a *static* method bag whose bodies use `this`. The ported
    ...
    // verify capture — the closures are stored on `this` (via decorateScaleMapper) and
...
// upstream runs this side-effecting registration at module import time. Swift libraries
