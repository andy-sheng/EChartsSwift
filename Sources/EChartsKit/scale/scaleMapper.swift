...
//   `DataSanitizationFilter` is now defined in data/helper/dataValueHelper.swift; the
//   former minimal forward-declared shim at the bottom of this file has been removed.
//
// note (the central deviation of this file): upstream models `ScaleMapper`/`LinearScaleMapper`/
...
// CONVENTIONS §2 prefers a `enum scaleMapper` namespace for the free functions, but every
 ...
 */
// `ScaleExtentKind = 0 | 1` modeled as `Int` (a genuine array index into `_extents`, CONVENTIONS §7).
...
// Swift dictionary key order is nondeterministic; the iteration order of
...
// modeled as a base `public` (not `open`) class of stored-closure "method slots" — see the
...
// declared `open` (not just `public`) because the sibling `open class Scale` subclasses it,
...
open class ScaleMapper {
    ...
    // ------------------------------------------------------------------------
    // upstream `interface LinearScaleMapper extends ScaleMapper` declares the two fields below;
     ...
     */
    // upstream `readonly _extents: number[][]` is SPARSE (index 0 always set, index 1 set only
...
// the `<THost>` self-type collapses — the sibling scales build this via a per-host factory
...
public func enableScaleMapperFreeze(_ host: ScaleMapper, _ subMapper: ScaleMapper) {
    ...
    if __DEV__ {
        host.freeze = {
            subMapper.freeze() // subMapper captured strongly (mirrors upstream; JS relies on GC).
        }
    }
}
...
// name-keyed slot copy/assignment helpers. Upstream copies via `obj[methodName]` dynamic
 ...
 */
// upstream `interface LinearScaleMapper extends ScaleMapper { _extents; _frozen }` — the
...
public func initLinearScaleMapper(
    ...
    //   modeled as the pre-sized length-2 `[[Double]?]` on the instance (see `_extents` note).
...
// `[unowned mapper]` breaks the retain cycle (the mapper owns the closures, the closures
...
// upstream `writeExtent(extentList: number[][], kind, start, end)` mutates the passed
...
private func writeExtent(
    ...
    if model.isValidBoundsForExtent(start, end) {
        ...
        if __DEV__ {
            ...
            // upstream `start != null && end != null` — start/end are non-optional Double here.
