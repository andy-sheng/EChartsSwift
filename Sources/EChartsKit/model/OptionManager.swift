...
//   import GlobalModel, { InnerSetOptionOpts } from './Global';     -> GlobalModel (util/types.swift placeholder; real type lands this phase). InnerSetOptionOpts -> `Any?` (see setOption PORT-TODO).
...
// PORT-TODO: `ParsedRawOption` is an upstream `interface` used as a plain data bag (no identity);
 ...
 */
// PORT-TODO: upstream `class OptionManager` (not subclassed / `export default`) -> `final class`
...
final class OptionManager {
    ...
    // PORT-TODO: upstream `rawOption: ECBasicOption` (structurally an `ECUnitOption` with optional
    ...
    // PORT-TODO: upstream `opt: InnerSetOptionOpts` (from model/Global). It is read only by the
    ...
    func setOption(
        ...
        if var optionBackup = optionBackup {
            ...
            //
            // PORT-TODO: upstream mutates the `_optionBackup` object in place (JS reference).
...
// PORT-TODO: upstream `real: number` — `realMap[realAttr]` may be `undefined` for an unknown attr;
