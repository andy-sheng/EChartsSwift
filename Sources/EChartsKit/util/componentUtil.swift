...
public enum component {
    ...
    public static func getUID(_ type: String) -> String {
        ...
        // PORT-TODO: `base` is a `Double` per CONVENTIONS §1; `String(Int(base))` reproduces the
    ...
    // PORT-TODO: upstream `SubTypeDefaulterManager` declares the two members as always-present
    ...
    public static func enableSubTypeDefaulter(_ target: SubTypeDefaulterManager & ClassManager) {
        ...
        // PORT-TODO: param types are left to inference so `defaulter` inherits the property's
        ...
        target.determineSubType = { (componentType: ComponentFullType, option: ComponentOption) -> String? in
            ...
            if type == nil {
                ...
                // PORT-TODO: `target.hasSubTypes` assumes `ClassManager` exposes a
    ...
    // PORT-TODO: upstream `TopologicalTravelable<T>` parameterizes only the callback's `this`/
    ...
    // PORT-TODO: upstream `DepGraphItem` is a mutable plain object shared through the `DepGraph`
    ...
    final class DepGraphItem {
        ...
        init() {
            predecessor = []
            successor = []
            // PORT-TODO: upstream leaves `originalDeps`/`entryCount` undefined until assigned
    ...
    public static func enableTopologicalTravel(
        ...
        entity.topologicalTravel = { (targetNameList, fullNameList, callback, context) throws in
            ...
            // PORT-TODO: ZRenderKit `util.each` provides only the array overload; iterate the
    ...
    // PORT-TODO: upstream `inheritDefaultOption<T, K>(superOption: T, subOption: K): K` is generic
