#!/bin/zsh
# Generate and capture every demo shown under the official gallery's `pie` section.
...
scenario_root="${1:-$repo_root/build/pie-interaction-scenarios}"
output_root="${2:-$repo_root/build/pie-interaction-visuals}"
...
"$binary" --interaction-generate-pie "$scenario_root" || exit 1

passed=()
failed=()
scenarios=("$scenario_root"/pie-all-*.json(N))
total=${#scenarios}
index=0
...
print "pie interaction captures: ${#passed} passed, ${#failed} failed, $total total"
if (( ${#failed} > 0 )); then
