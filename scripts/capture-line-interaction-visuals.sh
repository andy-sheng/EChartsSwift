#!/bin/zsh
# Generate and capture every demo shown under the official gallery's `line` section.
...
scenario_root="${1:-$repo_root/build/line-interaction-scenarios}"
output_root="${2:-$repo_root/build/line-interaction-visuals}"
...
"$binary" --interaction-generate-line "$scenario_root" || exit 1

passed=()
failed=()
scenarios=("$scenario_root"/line-all-*.json(N))
total=${#scenarios}
index=0
...
print "line interaction captures: ${#passed} passed, ${#failed} failed, $total total"
if (( ${#failed} > 0 )); then
