#!/bin/zsh
# Generate and capture every demo shown under the official gallery's `map` section.
...
scenario_root="${1:-$repo_root/build/map-interaction-scenarios}"
output_root="${2:-$repo_root/build/map-interaction-visuals}"
...
"$binary" --interaction-generate-map "$scenario_root" || exit 1

passed=()
failed=()
scenarios=("$scenario_root"/map-all-*.json(N))
total=${#scenarios}
index=0
...
print "map interaction captures: ${#passed} passed, ${#failed} failed, $total total"
if (( ${#failed} > 0 )); then
