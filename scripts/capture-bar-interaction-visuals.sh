#!/bin/zsh
# Generate and capture every demo shown under the official gallery's `bar` section.
...
scenario_root="${1:-$repo_root/build/bar-interaction-scenarios}"
output_root="${2:-$repo_root/build/bar-interaction-visuals}"
...
"$binary" --interaction-generate-bar "$scenario_root" || exit 1

passed=()
failed=()
scenarios=("$scenario_root"/bar-all-*.json(N))
total=${#scenarios}
index=0
...
print "bar interaction captures: ${#passed} passed, ${#failed} failed, $total total"
if (( ${#failed} > 0 )); then
