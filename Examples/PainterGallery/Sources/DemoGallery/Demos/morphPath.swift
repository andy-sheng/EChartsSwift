import ZRenderKit
// Faithful port of zrender test/morphPath.html  (canvas 1000x800)
//
// Upstream builds 7 morph targets — Rect, Circle, Isogon{n:6}, Sector, and 3 SVG-string paths
// (each createFromString(d) scaled into a BoundingRect via getBoundingRect().calculateTransform +
// applyTransform) — each styled fill:red / stroke:red / opacity:0.5. It then shows a SINGLE element
// on screen and morphPath()s it forever between random pairs (1000ms cubicInOut), re-firing from the
// done() callback. This port reproduces the same 7 targets and the continuous single-element morph
// loop; the random next-index is replaced by a deterministic cycle (+1) — a render-stable stand-in
// for Math.random() (CONVENTIONS: deterministic stand-ins are acceptable). A headless --render
// snapshot shows the first element (shapes[0], the Rect) at morph t=0.
extension DemoRegistry {
    static let demo_morphPath: Demo = Demo(
        name: "morphPath", category: "Path tools",
        summary: "morphPath loop over 7 shapes: Rect/Circle/Isogon/Sector + 3 SVG paths",
        width: 1000, height: 800
    ) { zr in
        // createPathFromString(d, x, y, width): SVG path scaled to fit BoundingRect(x,y,width,height),
        // preserving aspect (height derived from the path's own bounding rect) — exactly the HTML helper.
        func createPathFromString(_ d: String, _ x: Double, _ y: Double, _ width: Double) -> SVGPath {
            let path = createFromString(d)
            let pathRect = path.getBoundingRect()!
            let height = width / pathRect.width * pathRect.height
            let m = pathRect.calculateTransform(BoundingRect(x, y, width, height))
            path.applyTransform(m)
            return path
        }

        // Isogon{x:200, y:200, r:100, n:6}
        var isoShape = IsogonShape(); isoShape.x = 200; isoShape.y = 200; isoShape.r = 100; isoShape.n = 6
        let isogon = Isogon(); isogon.setShape(isoShape)

        // The 7 morph targets, in HTML order, each styled fill:red / stroke:red / opacity:0.5.
        let shapes: [Path] = [
            styled(rect(100, 100, 100, 200), fill: "red", stroke: "red", opacity: 0.5),
            styled(circle(200, 200, 90), fill: "red", stroke: "red", opacity: 0.5),
            styled(isogon, fill: "red", stroke: "red", opacity: 0.5),
            styled(sector(250, 200, 50, 100, 1, 3), fill: "red", stroke: "red", opacity: 0.5),
            styled(createPathFromString(D1, 100, 100, 200), fill: "red", stroke: "red", opacity: 0.5),
            styled(createPathFromString(D2, 100, 100, 200), fill: "red", stroke: "red", opacity: 0.5),
