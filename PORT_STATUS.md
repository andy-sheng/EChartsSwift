...
**Visual-parity campaign (July 2026) — native vs real-echarts.js `--compare` audit + systemic fixes.** Drove `swift run EChartsDemoGallery --compare` (renders each demo BOTH natively AND in real echarts.js 6.1 via WKWebView) over all 125 demos, fanned out a Workflow of per-demo comparison agents, and fixed the systemic bugs ranked by demos-affected. Result across ~14 commits (97f83f2…d62e2bb): **major discrepancies 73→~30, critical 31→~8, exact matches 26→~45+**. Fixes (each a whole category): (1) **stacking was 100% non-functional** — added `SeriesModel: DataStackSeriesModel` conformance (protocol-witness trap) + ported `processor/dataStack.swift`, wired into the slim data-processor stage; LineView now follows `getStackedDimension`. (2) **auto-legend never populated** — `modelUtil.isNameSpecified` was a stub returning false. (3) **attached labels never rendered** — the painter's `flattenDisplayList` never walked `_textContent`; fixed in `CALayerPainter` (enables sankey/treemap/sunburst/chord/tree/graph labels at once). (4) **class-1 black-fill** (`fill:null` dropped by `extendPathStyle`) cleared directly in RadarView/TreeView/ParallelView/LinesView. (5) **treemap/sankey black nodes** — ported the minimal VisualMapping each needs (treemap category-by-index; sankey linear `zrColor.lerp`) + `NSNull`-vs-`nil` guard in `buildVisuals` + render-time palette fallback. (6) **LineView** gained `areaStyle`, `smooth`, `step`; tree/graph/chord node-name labels; ThemeRiver smooth bands + layer labels. See [[visual-parity-traps]] memory. Remaining long tail (mostly minor/cosmetic or larger deferred features): visualMap calculable handles+endpoint labels, effectScatter ripple, geo maps, `textConfig:'inside'` label centering, themeRiver time-axis label formatting, axisLabel/axisLine subpixel color perception.

**Phase 53 (UNIVERSAL TRANSITION — cross-series MORPHING; the `map-bar-morph` / `bar-drilldown` / `scatter-aggregate-bar` family of official examples now morphs instead of cross-fading): two series that share an `id` (or a `universalTransition.seriesKey`) and set `universalTransition: true` now MORPH into each other across a `setOption`, through zrender's already-ported `morphPath`/`combineMorph`/`separateMorph`. Files added: `core/lifecycle.swift` ← `core/lifecycle.ts` (the typed lifecycle emitter + `UpdateLifecycleParams`; `registerUpdateLifecycle` is added to `EChartsExtensionInstallRegisters` here, mirroring echarts.ts:3091), `animation/morphTransitionHelper.swift` ← `animation/morphTransitionHelper.ts` (`applyMorphAnimation` + `getPathList` + the `clone`/`split` path dividers), `animation/universalTransition.swift` ← `animation/universalTransition.ts` (785 lines: the groupId/childGroupId direction rule, the `DataDiffer('multiple')`-driven one-to-one / many-to-one / one-to-many / many-to-many morph dispatch, the per-`ExtensionAPI` `makeInner` old-series store that survives a `notMerge` model rebuild, `findTransitionSeriesBatches`, `installUniversalTransition`). Files edited: `core/ECharts.swift` — `renderSeries` now emits the four faithful lifecycle triggers (`series:beforeupdate` / `series:layoutlabels` / `series:transition` / `series:afterupdate`, echarts.ts:2483-2538) with an `updateParams` carrying `updatedSeries`, `setOption` threads `optionChanged: true` (echarts.ts:779) — the flag that GATES the transition — `update()` emits `coordsys:aftercreate` + `afterupdate`, and `installOnce` calls `installUniversalTransition(_registers)`; `animation/basicTransition.swift` — `saveOldStyle`/`getOldStyle` were no-op stubs, now a real `makeInner`-backed store, plus the `AnimateOrSetPropsOption` (object) form of `initProps` for its `isFrom` flag; `chart/bar/PictorialBarView.swift` + `chart/treemap/TreemapView.swift` — the two upstream `(el as ECElement).disableMorphing = true` sites are now wired (via the `getMorphInner` side store, the `states.swift` `HighDownInner` pattern). **Clean build; `swift test` — 702 / 0 failures / 58 skipped** (baseline 697; +5 `UniversalTransitionTests`: map→bar and bar→map both produce real morph targets — `isMorphing`/`isCombineMorphing` true on every incoming Path — stepping the animation clip moves the bounding rect from the source region polygon to the bar's final rect; the MANY-TO-ONE `combineMorph` branch (2 scatter series with `dataGroupId` → 1 bar series with an ARRAY `seriesKey` + `divideShape:'clone'` — the `scatter-aggregate-bar` shape) and the ONE-TO-MANY `separateMorph` branch (a parent bar with `groupId` → the child series' bars via `dataGroupId` — the `bar-drilldown` shape) each produce real morph targets; and a series WITHOUT `universalTransition` does not morph). Verified against the REAL `official-map-bar-morph` option pair (USA.json, 52 states): all 52 bars morph. DEFERRED (documented `PORT-NOTE`s): `setOption`'s `transition`/`SetOptionOpts` finder form (the `seriesTransition` param is plumbed through `UpdateLifecycleParams` and `transitionSeriesFromOpt` is ported, but `ECharts.setOption` has no opts bag to supply it — the id/seriesKey-driven form, which every official example uses, is fully wired); the port's label-layout stage still runs after `series:transition` rather than as a `series:layoutlabels` listener (harmless — labels are never morph endpoints).**
...
## 78. Phase 47 — Brush lineX / lineY (1-D band brush types; extends Phase 44)

**Goal (met):** the brush supports `brushType:'lineX'` (vertical x-band) and `'lineY'` (horizontal y-band)
alongside `'rect'`. **Clean build (0 warnings); `swift test` — 302 / 0 / 58** (baseline 301; +1). Demos 101/101.

`brushVisual.swift`: added `lineX`/`lineY` to `makeBrushCommonSelectorForSeries` — point selector tests the
datum coordinate against the 1-D `[min,max]` band; rect selector tests x/y overlap. `brushRange1D` reads the
flat 1-D range (lineX/lineY need no 2-D boundingRect). `EChartsView._finishBrushDrag` reads the brush
component's `brushType` option and dispatches a 1-D (line) or 2-D (rect) range. Deferred: polygon brush,
coordRange persistence for lineX/lineY, the toolbox brush button.
